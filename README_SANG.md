...
#### 3.1. Dự án này có sử dụng MVC không?
**Câu trả lời là: CÓ, nhưng là một biến thể hiện đại.**
Truyền thống Next.js không ép buộc MVC, nhưng để dự án lớn dễ bảo trì, chúng ta đã chủ động áp dụng quy tắc MVC:
- **Model**: Database Schema (Supabase) + Typescript Interfaces (`types/`).
- **View**: Các React Components (`app/`, `components/`).
- **Controller**: Chính là **Server Actions** (`server/actions/`). Thay vì tạo API Routes (`pages/api`) rườm rà, Server Action đóng vai trò là Controller nhận tín hiệu từ View, điều phối cho Service xử lý và trả về kết quả.

#### 3.2. Tại sao chọn cấu trúc thư mục phân tách (`server/` vs `app/`) thay vì cấu trúc cũ?
...
#### 3.3. Ranh giới FE/BE và Tối ưu hiệu suất trong Next.js App Router
...
#### 3.4. Sơ đồ Cây Thư mục (Project Folder Structure)
