...
export default async function EditEmployeePage({ params }: PageProps) {
  ...
  const [departments, employee, contracts, assets, availableAssets, salaryHistory] = await Promise.all([
    employeeService.getDepartments(),
    employeeService.getEmployee(employeeId),
    contractRepo.getContractsByEmployeeId(employeeId).catch(() => []),
    assetRepo.getAssetsByEmployeeId(employeeId).catch(() => []),
    assetRepo.getAssets().then(all => all?.filter(a => a.status === 'Sẵn sàng' || a.status === 'Available') || []).catch(() => []),
