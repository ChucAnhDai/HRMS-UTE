...
export async function createEmployeeAction(prevState: unknown, formData: FormData) {
  try {
    ...
    const newEmp = await employeeService.createEmployee(formData)
    
    await activityService.logActivity('THÊM MỚI', 'Nhân viên', newEmp?.id, `Thêm nhân viên ${rawData.last_name || ''} ${rawData.first_name || ''}`.trim())
