...
import { Building2, Plus, Edit2, Trash2, Users, Search, AlertCircle, Loader2 } from "lucide-react"
...
export default function DepartmentTableView({ departments, canManage }: Props) {
  ...
  const handleDelete = async (id: number, name: string) => {
    if (!confirm(`Bạn có chắc chắn muốn xóa phòng ban "${name}"? Hành động này không thể hoàn tác.`)) {
      return
    }
          ...
          <table className="w-full text-left">
            <thead className="bg-gray-50 border-b border-gray-100">
              <tr>
                ...
                <th className="px-6 py-4 text-sm font-bold text-gray-700">Tên phòng ban</th>
                <th className="px-6 py-4 text-sm font-bold text-gray-700 text-center">Số nhân viên</th>
            ...
            <tbody className="divide-y divide-gray-50">
              {filteredDepartments.length === 0 ? (
                <tr>
                  <td colSpan={canManage ? 5 : 4} className="text-center py-12 text-gray-500">
                ...
                <tr key={dept.id} className="hover:bg-gray-50 transition-colors group">
                  <td className="px-6 py-4 text-sm text-gray-600 font-mono">#{dept.id}</td>
