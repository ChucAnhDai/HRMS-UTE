...
const eventCategories = [
  { label: 'Đi làm đúng giờ', color: 'bg-green-100 text-green-800', icon: Clock },
  { label: 'Đi muộn / Về sớm', color: 'bg-orange-100 text-orange-800', icon: Clock },
...
export default function CalendarView({ events, employees }: CalendarViewProps) {
  ...
  const handleCheckIn = async () => {
    ...
    try {
        const { checkInAction } = await import('@/server/actions/attendance-actions')
  ...
  const renderEventContent = (eventInfo: any) => {
    return (
      <div className={cn(
        ...
        eventInfo.event.extendedProps.type === 'Leave' ? 'bg-blue-200 border-l-2 border-blue-600' :
        eventInfo.event.extendedProps.status === 'Present' ? 'bg-green-200 border-l-2 border-green-600' :
        eventInfo.event.extendedProps.status === 'Late' ? 'bg-orange-200 border-l-2 border-orange-600' :
        'bg-red-200 border-l-2 border-red-600'
      )}>
        {eventInfo.timeText && <span className="mr-1 opacity-75 font-medium">{eventInfo.timeText}</span>}
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col gap-2">
        ...
        <h3 className="text-xl font-extrabold text-black">Lịch chấm công & Sự kiện</h3>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
        ...
        <div className="space-y-6">
          <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 space-y-4">
            ...
            <div className="flex flex-col gap-2">
                <label className="text-xs font-bold text-black uppercase">Chọn nhân viên chấm công</label>
                    ...
                    {searchTerm && (
                        <div className="absolute top-full left-0 right-0 mt-1 max-h-48 overflow-y-auto bg-white border border-gray-200 rounded-lg shadow-lg z-10">
                            {filteredEmployees.length > 0 ? filteredEmployees.map(emp => (
                                ...
                                >
                                    <span className="font-bold">#{emp.id}</span> - {emp.last_name} {emp.first_name}
                                </div>
                            )) : (
            ...
            >
              {loading ? <Clock className="h-4 w-4 animate-spin" /> : <Plus className="h-4 w-4" />}
            ...
            <div className="mt-8 space-y-4">
              <h4 className="text-sm font-bold text-black uppercase tracking-wider">Chú thích</h4>
        ...
        <div className="lg:col-span-3">
          <div className="bg-white rounded-2xl shadow-sm overflow-hidden border border-gray-100 p-4">
            <style jsx global>{`
                ...
                .fc .fc-button-primary { background-color: white; color: #4b5563; border: 1px solid #e5e7eb; font-weight: 600; text-transform: capitalize; padding: 0.5rem 1rem; }
                .fc .fc-button-primary:hover { background-color: #f9fafb; color: #111827; }
                .fc .fc-button-primary:disabled { opacity: 0.5; }
                .fc .fc-button-group > .fc-button { border-radius: 0.5rem; margin-left: 4px; }
                .fc .fc-button-active { background-color: #eff6ff !important; color: #2563eb !important; border-color: #dbeafe !important; }
                .fc .fc-daygrid-day-number { font-size: 0.875rem; font-weight: 600; color: #4b5563; padding: 0.5rem; text-decoration: none; }
                .fc .fc-col-header-cell-cushion { font-size: 0.75rem; font-weight: 700; text-transform: uppercase; color: #9ca3af; padding: 1rem 0; }
