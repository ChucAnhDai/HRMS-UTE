...
import { updateContractAction, deleteContractAction } from '@/server/actions/contract-actions'
import { useActionState, useState, useTransition } from 'react'
import { Plus, FileText, CheckCircle, AlertCircle, Edit2, Trash2, X } from 'lucide-react'
      ...
      <div className="p-6">
        ...
        {showForm && (
            <form action={formAction} className="mb-8 bg-blue-50/50 p-4 rounded-lg border border-blue-100">
                ...
                {editingContract && <input type="hidden" name="contract_id" value={editingContract.id} />}
                ...
                {state?.success && !isPending && (
                    <div className="mb-4 text-sm text-green-600 flex items-center gap-1">
                        <CheckCircle className="w-4 h-4" /> {editingContract ? 'Cập nhật' : 'Thêm'} hợp đồng thành công!
                    </div>
                )}
                ...
                <div className="flex justify-end gap-2">
                    ...
                    >
                        {isPending ? 'Đang lưu...' : editingContract ? 'Cập nhật' : 'Lưu hợp đồng'}
                    </button>
                </div>
            </form>
        )}
        ...
        {contracts.length === 0 ? (
            ...
            <div className="space-y-3">
                {contracts.map((contract) => (
                    <div key={contract.id} className="p-4 bg-gray-50 rounded-lg border border-gray-100 hover:border-blue-200 transition-colors">
                        ...
                        <div className="pt-3 border-t border-gray-200">
                            <p className="text-sm text-gray-700">
                                ...
                                <span className="text-blue-600 font-bold">
                                    {contract.salary ? contract.salary.toLocaleString('vi-VN') : '0'} VND
                                </span>
                            </p>
                            {contract.notes && (
