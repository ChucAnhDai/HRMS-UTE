...
export default function EditPayslipModal({ payslip, onClose }: Props) {
  ...
  useEffect(() => {
    ...
    const estimatOtSalary = Math.round(
      hourlyRate * 1.5 * Number(formData.ot_hours),
    );
    ...
    setFormData((prev) => {
      if (
        prev.ot_hours !== payslip.ot_hours &&
        prev.ot_salary === payslip.ot_salary
      ) {
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="space-y-2">
                ...
                <div className="w-full px-4 py-3 bg-gray-100 border border-gray-200 rounded-lg text-gray-700 font-medium select-none text-sm">
                  {payslip.employees?.last_name} {payslip.employees?.first_name}{" "}
                  <span className="text-gray-500 font-normal">
                    (MNV: #{payslip.employee_id})
                  </span>
                </div>
              </div>
              <div className="space-y-2">
